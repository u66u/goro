...
void* stack_pool_get(stack_pool_t* pool) {
    if (!pool || !pool->freelist_head) {
        fprintf(
            stderr,
            "could not get a free node from the pool because the pool is NULL "
            "or the node is NULL. Make sure you're not over pool's capacity");
        return NULL;
    }
...
void stack_pool_return(stack_pool_t* pool, void* stack) {
    if (!pool || !stack) {
        fprintf(stderr, "could not return stack to pool because either pool or "
                        "stack is NULL");
        return;
    }
